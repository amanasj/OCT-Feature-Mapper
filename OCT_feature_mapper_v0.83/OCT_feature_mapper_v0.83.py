...
PREVIEW_MAX_WIDTH = 442
PREVIEW_MAX_HEIGHT = 718
...
class OCTEZAreaWorkflow:
    ...
    def show_instructions(self, title: str, lines: List[str], size: Tuple[int, int] = (230, 480)) -> None:
        ...
        y = 28
        for line in lines:
            cv2.putText(img, line, (15, y), cv2.FONT_HERSHEY_SIMPLEX, 0.52, (0, 0, 0), 1)
            y += 26
    ...
    def _store_current_points_for_bscan(self, idx: int) -> None:
    ...
    def _build_live_enface_preview(self, target_height: int) -> np.ndarray:
        if self._enface_full is None:
            return np.zeros((target_height, max(120, PREVIEW_MAX_WIDTH), 3), dtype=np.uint8)
        ...
        scale = min(PREVIEW_MAX_WIDTH / float(w), PREVIEW_MAX_HEIGHT / float(h), target_height / float(h))
        scale = max(min(scale, 1.0), 0.1)
        resized = cv2.resize(preview, None, fx=scale, fy=scale, interpolation=cv2.INTER_AREA if scale < 1.0 else cv2.INTER_LANCZOS4)

        panel_h = max(target_height, resized.shape[0])
        panel_w = resized.shape[1]
        panel = np.ones((panel_h, panel_w, 3), dtype=np.uint8) * 18
        y_off = max((panel_h - resized.shape[0]) // 2, 0)
        panel[y_off:y_off + resized.shape[0], 0:panel_w] = resized
    ...
    def prepare_bscan_display(self, img_original: np.ndarray) -> np.ndarray:
        ...
        preview_panel = self._build_live_enface_preview(display.shape[0])
...
class OCTEZAreaGUI:
    ...
    def _build_ui(self) -> None:
        ...
        instructions_text = (
            ...
            "3. In the B-scan window, click EZ points along the band (up to 6), then press Enter to save.\n"

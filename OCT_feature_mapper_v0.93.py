...
APP_TITLE = "OCT feature mapper v0.93"
...
POLYGON_PANEL_WIDTH = 250
POLYGON_PANEL_PADDING = 16
POLYGON_BUTTON_HEIGHT = 50
...
class SessionResult:
    ...
    ez_perimeter_mm: Optional[float] = None
...
class OCTEZAreaWorkflow:
    ...
    def _enface_display_scale_factors(self) -> Tuple[float, float]:
        ...
        if base_scale <= 0:
            return 1.0, 1.0
        return (
            self._metadata.scale_x_enface / base_scale,
            self._metadata.scale_y_enface / base_scale,
        )
    ...
    def _build_polygon_control_panel(self, panel_height: int) -> np.ndarray:
        ...
        button_specs = [
            ("save", "Save", "Save overlay + CSV"),
            ("next_polygon", "Next Polygon", "Finish current polygon"),
            ("coregister", "Coregister Image", "Browse + align image"),
        ]

        button_y = 92
        for key, label, subtitle in button_specs:
            ...
            fill = POLYGON_BUTTON_PRESSED_FILL if is_pressed else POLYGON_BUTTON_HOVER_FILL if is_hovered else POLYGON_BUTTON_FILL
            border = POLYGON_BUTTON_PRESSED_BORDER if is_pressed else POLYGON_BUTTON_HOVER_BORDER if is_hovered else POLYGON_BUTTON_BORDER
        ...
        for line in info_lines:
            if line == "":
                info_y += 10
                continue
            colour = POLYGON_PANEL_TEXT if line in {"Shortcuts", "Coregistration"} else POLYGON_PANEL_MUTED
            thickness = 2 if colour == POLYGON_PANEL_TEXT else 1
            font_scale = 0.52 if thickness == 2 else 0.43
    ...
    def _save_overlay_snapshot(self, overlay: np.ndarray, notify: bool = True) -> bool:
        if self._area is None or self._folder is None or self._metadata is None:
            self.report("No polygon area is available to save yet.")
            return False
        if self._coregistered_enface_full is not None:
    ...
    def _append_nearest_polygon_vertex(self, x: int, y: int) -> None:
        if not self._points_px_crop:
            return
        ...
        x_crop = x_unscaled - self._v1
        y_crop = y_unscaled - self._h1
    ...
    def on_polygon_click(self, event: int, x: int, y: int, flags: int, param: object) -> None:
        ...
        if event == cv2.EVENT_LBUTTONUP and self._coregistration_drag_active:
            ...
            self._coregistration_drag_moved = False
            if should_add_point:
                self._append_nearest_polygon_vertex(x, y)
            elif moved:
                self._apply_coregistration_transform()
            return
    ...
    def run_polygon_overlay(self) -> str:
        ...
        self.show_instructions(
            ...
            [
                ...
                "Left panel = Save / Next / Coregister",
                "u = Undo last point",
        ...
        while True:
            ...
            panel = self._build_polygon_control_panel(overlay.shape[0])
            panel = cv2.resize(panel, (POLYGON_PANEL_WIDTH, display_overlay.shape[0]), interpolation=cv2.INTER_NEAREST)
            ...
            elif key == ord("u") and self.selected_vertices:
    ...
    def save_session_outputs(self) -> SessionResult:
        ...
        with csv_path.open("w", newline="", encoding="utf-8") as fh:
            ...
            writer.writerow(
                ["bscan_index"]
                + [f"x_pixel_{i}" for i in range(1, MAX_EZ_POINTS_PER_BSCAN + 1)]
        ...
        measurements_file = "ez_measurements.csv"
        ...
        with measurements_path.open("w", newline="", encoding="utf-8") as fh:
            ...
            writer.writerow(["labelled_bscan_count", len(self.ez_coords), "count"])
            for idx, measurement in enumerate(polygon_measurements, start=1):
                writer.writerow([f"polygon_{idx}_area", f"{measurement.area_mm2:.6f}", "mm^2"])
                writer.writerow([f"polygon_{idx}_perimeter", f"{measurement.perimeter_mm:.6f}", "mm"])
                writer.writerow([f"polygon_{idx}_centroid_x_px", f"{measurement.centroid_px[0]:.3f}", "px"])
                writer.writerow([f"polygon_{idx}_centroid_y_px", f"{measurement.centroid_px[1]:.3f}", "px"])
                writer.writerow([f"polygon_{idx}_centroid_x_mm", f"{measurement.centroid_mm[0]:.6f}", "mm"])
                writer.writerow([f"polygon_{idx}_centroid_y_mm", f"{measurement.centroid_mm[1]:.6f}", "mm"])
            writer.writerow(["ez_area", f"{total_area:.6f}" if total_area is not None else "", "mm^2"])
            writer.writerow(["ez_perimeter", f"{total_perimeter:.6f}" if total_perimeter is not None else "", "mm"])
        ...
        result = SessionResult(
            ...
            ez_area_mm2=total_area,
            ez_perimeter_mm=total_perimeter,
...
#            json.dump(asdict(result), fh, indent=2)
        ...
        self.report(f"Saved ez_measurements.csv in {self._folder}")
        return result
...
class OCTEZAreaGUI:
    def __init__(self) -> None:
        ...
        self.root.geometry("936x744")
        self.root.minsize(888, 696)
    ...
    def _on_success(self, result: SessionResult) -> None:
        ...
        if result.ez_area_mm2 is None:
            ...
            self.result_var.set(
                ...
                f"EZ perimeter: {result.ez_perimeter_mm:.3f} mm\n"

...
APP_TITLE = "OCT feature mapper v0.97"
...
class OCTFeatureAreaWorkflow:
    def __init__(
        ...
        self._polygon_panel_width_current = POLYGON_PANEL_WIDTH
    ...
    def show_instructions(
        ...
        title: str,
        lines: List[str],
        size: Optional[Tuple[int, int]] = (230, 480),
        ...
        if size is None:
            max_width = 0
            for line in lines:
                text_width, _ = cv2.getTextSize(line, cv2.FONT_HERSHEY_SIMPLEX, font_scale, 1)[0]
                max_width = max(max_width, text_width)
            size = (
                max(margin_top + margin_bottom + len(lines) * line_gap, 160),
                max(max_width + (2 * margin_x), 220),
            )
        render_height = int(size[0] * supersample)
        render_width = int(size[1] * supersample)
        img_large = np.ones((render_height, render_width, 3), dtype=np.uint8) * 255
        y = margin_top * supersample
        for line in lines:
            cv2.putText(
                img_large,
                line,
                (margin_x * supersample, y),
            ...
            y += line_gap * supersample
        img = cv2.resize(img_large, (size[1], size[0]), interpolation=cv2.INTER_AREA)
    ...
    def load_metadata(self, xml_path: Path) -> OCTMetadata:
        ...
        return OCTMetadata(
            ...
            enface_width_px=int(localizer_full.Columns),
            enface_height_px=int(localizer_full.Rows),
            res_x=res_x,
            start_x=start_x,
            y_coords=y_coords,
    ...
    def _polygon_ui_layout(self, enface_width_px: int, enface_height_px: int) -> dict[str, float | int]:
        reference_dim = max(1, min(int(enface_width_px), int(enface_height_px)))
        scale = max(0.52, min(math.sqrt(reference_dim / 1500.0), 1.0))
        high_res_boost = 1.2 if reference_dim >= 1200 else 1.0
        high_res_text_boost = 1.2 if reference_dim >= 1200 else 1.0
        button_height_boost = 1.2
        panel_width = max(210, min(500, int(round(POLYGON_PANEL_WIDTH * scale))))
        padding = max(10, min(28, int(round(POLYGON_PANEL_PADDING * scale))))
        button_height = max(41, min(132, int(round(POLYGON_BUTTON_HEIGHT * scale * high_res_boost * button_height_boost))))
        button_gap = max(6, min(18, int(round(POLYGON_BUTTON_GAP * scale))))
        title_font = max(0.44, min(1.56, 0.90 * scale * high_res_text_boost))
        subtitle_font = max(0.28, min(0.92, 0.50 * scale * high_res_text_boost))
        button_font = max(0.34, min(1.15, 0.66 * scale * high_res_text_boost))
        button_sub_font = max(0.24, min(0.76, 0.44 * scale * high_res_text_boost))
        info_header_font = max(0.34, min(1.00, 0.58 * scale * high_res_text_boost))
        info_body_font = max(0.28, min(0.82, 0.47 * scale * high_res_text_boost))
        info_line_gap = max(14, min(34, int(round(24 * scale))))
        section_gap = max(button_height // 2, 12)
        title_y = max(24, int(round(38 * scale)))
        subtitle_y = max(title_y + max(18, int(round(22 * scale))), int(round(66 * scale)))
        button_start_y = max(subtitle_y + max(18, int(round(30 * scale))), int(round(102 * scale)))
        shadow_offset = max(2, int(round(4 * scale)))
        shadow_inset = max(1, int(round(2 * scale)))
        label_y_offset = max(18, min(52, int(round(25 * scale * high_res_boost * high_res_text_boost))))
        subtitle_y_offset = max(30, min(84, int(round(47 * scale * high_res_boost * high_res_text_boost))))
        info_top_gap = max(10, min(28, int(round(18 * scale))))
        help_font_scale = max(0.24, min(1.18, (0.42 * scale + 0.04) * high_res_boost * high_res_text_boost))
        help_line_gap = max(14, min(50, int(round(22 * scale * high_res_boost * high_res_text_boost))))
        help_margin_x = max(12, min(28, int(round(15 * scale))))
        help_margin_top = max(18, min(38, int(round(28 * scale))))
        help_margin_bottom = max(14, min(30, int(round(20 * scale))))
        return {
            "scale": scale,
            "high_res_boost": high_res_boost,
            "high_res_text_boost": high_res_text_boost,
            ...
            "button_sub_font": button_sub_font,
            "info_header_font": info_header_font,
            "info_body_font": info_body_font,
            "info_line_gap": info_line_gap,
            "section_gap": section_gap,
            "title_y": title_y,
            "subtitle_y": subtitle_y,
            "button_start_y": button_start_y,
            "shadow_offset": shadow_offset,
            "shadow_inset": shadow_inset,
            "label_y_offset": label_y_offset,
            "subtitle_y_offset": subtitle_y_offset,
            "info_top_gap": info_top_gap,
            "help_font_scale": help_font_scale,
            "help_line_gap": help_line_gap,
            "help_margin_x": help_margin_x,
            "help_margin_top": help_margin_top,
            "help_margin_bottom": help_margin_bottom,
        }
    ...
    def _build_polygon_control_panel(self, panel_height: int, enface_width_px: int, enface_height_px: int) -> np.ndarray:
        layout = self._polygon_ui_layout(enface_width_px, enface_height_px)
        panel_width = int(layout["panel_width"])
        padding = int(layout["padding"])
        button_height = int(layout["button_height"])
        button_gap = int(layout["button_gap"])
        shadow_offset = int(layout["shadow_offset"])
        shadow_inset = int(layout["shadow_inset"])
        label_y_offset = int(layout["label_y_offset"])
        subtitle_y_offset = int(layout["subtitle_y_offset"])
        info_top_gap = int(layout["info_top_gap"])
        info_line_gap = int(layout["info_line_gap"])
        section_gap = int(layout["section_gap"])
        supersample = 2
        panel_large = np.full((panel_height * supersample, panel_width * supersample, 3), POLYGON_PANEL_BACKGROUND, dtype=np.uint8)
        self._polygon_panel_width_current = panel_width
        self._polygon_button_rects = {}
        now = time.monotonic()
        press_offset = POLYGON_BUTTON_PRESSED_OFFSET
        ...
        cv2.putText(
            panel_large,
            "Overlay Tools",
            (ss(padding), ss(int(layout["title_y"]))),
            cv2.FONT_HERSHEY_SIMPLEX,
            float(layout["title_font"]) * supersample,
            POLYGON_PANEL_TEXT,
            max(1, 2 * supersample),
            cv2.LINE_AA,
        )
        cv2.putText(
            ...
            (ss(padding), ss(int(layout["subtitle_y"]))),
            cv2.FONT_HERSHEY_SIMPLEX,
            float(layout["subtitle_font"]) * supersample,
            POLYGON_PANEL_MUTED,
            max(1, supersample),
            cv2.LINE_AA,
        )
        ...
        button_specs = [
            ...
            ("exit", "Exit", "Finish session", False),
        ]

        button_y = int(layout["button_start_y"])
        for key, label, subtitle, add_section_gap in button_specs:
            x1 = padding
            x2 = panel_width - padding
            y1 = button_y
            y2 = y1 + button_height
            ...
            y_offset = press_offset if is_pressed else 0
            if not is_pressed:
                cv2.rectangle(
                    panel_large,
                    (ss(x1 + shadow_inset), ss(y1 + shadow_offset)),
                    (ss(x2 + shadow_inset), ss(y2 + shadow_offset)),
                    (18, 18, 20),
                    -1,
                )
            cv2.rectangle(panel_large, (ss(x1), ss(y1 + y_offset)), (ss(x2), ss(y2 + y_offset)), fill, -1)
            cv2.rectangle(panel_large, (ss(x1), ss(y1 + y_offset)), (ss(x2), ss(y2 + y_offset)), border, max(1, 2 * supersample))
            cv2.putText(
                panel_large,
                label,
                (ss(x1 + padding), ss(y1 + y_offset + label_y_offset)),
                cv2.FONT_HERSHEY_SIMPLEX,
                float(layout["button_font"]) * supersample,
                POLYGON_PANEL_TEXT,
                max(1, 2 * supersample),
                cv2.LINE_AA,
            )
            cv2.putText(
                panel_large,
                subtitle,
                (ss(x1 + padding), ss(y1 + y_offset + subtitle_y_offset)),
                cv2.FONT_HERSHEY_SIMPLEX,
                float(layout["button_sub_font"]) * supersample,
                POLYGON_PANEL_MUTED,
                max(1, supersample),
                cv2.LINE_AA,
            )
            button_y = y2 + button_gap
            if add_section_gap:
                button_y += section_gap

        info_y = button_y + info_top_gap
        ...
        for line in info_lines:
            ...
            font_scale = float(layout["info_header_font"]) if thickness == 2 else float(layout["info_body_font"])
            cv2.putText(
                panel_large,
                line,
                (ss(padding), ss(info_y)),
                cv2.FONT_HERSHEY_SIMPLEX,
                font_scale * supersample,
                colour,
                max(1, thickness * supersample),
                cv2.LINE_AA,
            )
            info_y += info_line_gap
        return cv2.resize(panel_large, (panel_width, panel_height), interpolation=cv2.INTER_AREA)
    ...
    def _overlay_display_to_full_point(self, x: int, y: int) -> Optional[Tuple[int, int]]:
        if self._enface_full is None:
            return None
        x_image = x - self._polygon_panel_width_current
        if x_image < 0:
            return None
    ...
    def on_polygon_click(self, event: int, x: int, y: int, flags: int, param: object) -> None:
        ...
        if x < self._polygon_panel_width_current:
            return
    ...
    def run_polygon_overlay(self) -> str:
        ...
        layout_width = self._metadata.enface_width_px if self._metadata is not None else width
        layout_height = self._metadata.enface_height_px if self._metadata is not None else height
        help_layout = self._polygon_ui_layout(layout_width, layout_height)
        ...
        self.show_instructions(
            ...
            [
                ...
                "q = Exit overlay session",
            ],
            size=None,
            font_scale=float(help_layout["help_font_scale"]),
            line_gap=int(help_layout["help_line_gap"]),
            margin_x=int(help_layout["help_margin_x"]),
            margin_top=int(help_layout["help_margin_top"]),
            margin_bottom=int(help_layout["help_margin_bottom"]),
        )
        ...
        while True:
            ...
            panel = self._build_polygon_control_panel(display_overlay.shape[0], layout_width, layout_height)
